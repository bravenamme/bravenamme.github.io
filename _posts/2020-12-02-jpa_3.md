...
팀과 회원의 일대대 관계설정을 위해서 팀 엔티티에 컬렉션인 List<Member> member 를 추가했다.
...
2. 하지만 객체의 관점에서만 보면 순수한 객체 양쪽 방향 모두에 값을 입력 하는것이 가장 안전하다. 반대방향에 연관관계를 설정하지 않은 상태로 회원을 출력해보면 결과는 "0" 이 나온다.
